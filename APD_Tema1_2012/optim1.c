...
void main(int argc, char* argv[]) {
	...
	for (an = 1; an <= nrAni; an++) {
		...
		/* Pentru determinarea minimului, procedam astfel:
			...
			Intotdeau a procedam ca la primul caz. Alegem cel mai mic element, daca unul dintre ele este mai mic si inlocuim valoarea de pe pozitia i,j cu valoarea celui mai mic element + 1*/
		int x, y;
		for (i = 0; i < n; i++) {
			for ( j = 0; j < n; j++) {
